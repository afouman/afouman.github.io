...
A static event-menu and real-time ordering app designed for GitHub Pages. Guests first create a temporary profile with their name and phone number, then RSVP Yes, Maybe, or No, and finally order if they are attending. Firebase gives their browser a temporary anonymous identity; no verification code is sent. Profile, RSVP, and orders are separate records tied to that identity. Only Yes RSVPs can order while ordering is open, while RSVP stays available after ordering closes. Active orders lock RSVP downgrades until the order is cancelled, rejected, or fully served. Hosts sign in with Google to manage events, menus, orders, and a live RSVP panel.
